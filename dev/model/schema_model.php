  ...
  public function createTable($table_name,$params){
    ...
    foreach($params as $p){
      ...
      if($p->type == 'string' || $p->type == 'varchar'){
        if(!isset($p->length)) $p->length = 255;
        if($p->length > 255){
          $p->type = 'text';
          $sql .= ' '.$p->type.'';
          $is_text = 1;
      ...
      if(isset($p->is_unsigned) && !empty($p->is_unsigned)) $sql .= ' UNSIGNED';
