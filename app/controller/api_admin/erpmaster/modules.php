...
class Modules extends JI_Controller{
	...
	public function index(){
		...
		$dcount = $this->amod->countAll($keyword);
		$ddata = $this->amod->getAll($page,$pagesize,$sortCol,$sortDir,$keyword);
	...
	public function reload(){
		...
		$allowed_all = 0;
		$modules = array();
		$sess->admin->modules = $this->apmm->getUserModules($sess->admin->id);
		...
		if($allowed_all){
			...
			foreach($parmod as $pm){
				...
				$chimod = $this->amod->getChild($pm->identifier);
				if(count($chimod)>0){
