...
* API_Front/kandidat
*/
class Organisasi extends JI_Controller {
  public $currentDataProgress = 'Riwayat Organisasi &amp; Profesi';
  ...
  public function __construct()
  {
    ...
    $this->load("api_front/b_user_orghistory_model", 'buohm');
  ...
  private function _updateDataProgress($bum,$c_apply_id=''){
    ...
    $capm = $this->capm->check($bum->id, $this->currentDataProgress, 'data');
    if(isset($capm->id)){
  ...
  public function index()
  {
    ...
    $data=$this->buohm->getByUserId($dt['sess']->user->id);
  ...
  public function baru(){
    ...
    if(empty(strlen($lokasi))){
      $this->status = 1102;
      $this->message = 'Lokasi harus diisi';
      $this->__json_out($data);
    }

    if(strlen($tahun_sampai) != 4){
      $this->status = 1104;
      $this->message = 'Tahun sampai harus diisi';
      $this->__json_out($data);
    }
    ...
    $this->_setULog($dt['sess']->user->id, 13, 'organisasi');
  ...
  public function detail($id)
  {
    ...
    $data = $this->buohm->getByIdUserId($id,$dt['sess']->user->id);
    if(!isset($data->id)){
      $this->status = 1114;
  ...
  public function edit($id){
    ...
    $buohm = $this->buohm->getByIdUserId($id,$dt['sess']->user->id);
    if(!isset($buohm->id)){
      $this->status = 1114;
      $this->message = 'This data not belong to this user';
      $this->__json_out($data);
