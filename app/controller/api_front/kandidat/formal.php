...
* API_Front/kandidat
*/
class Formal extends JI_Controller {
  public $data_utype = 'Formal';
  public $currentDataProgress = 'Riwayat Pendidikan Formal';
  ...
  private function _updateDataProgress($bum,$c_apply_id=''){
    ...
    $counter = count($this->bupdm->getByUserId($bum->id, $this->data_utype));
    if($counter <= 2){
      $progress['to_val'] = 2;
      $progress['from_val'] = $counter;
    }else{
  ...
  public function index()
  {
    ...
    $data=$this->bupdm->getByUserId($dt['sess']->user->id, $this->data_utype);
  ...
  public function baru(){
    ...
    if(intval($bum->apply_statno) > 4 && intval($bum->apply_statno) <= 7){
      $this->status = 951;
      $this->message = 'Maaf!!! Anda sudah tidak bisa merubah data pada tahap ini';
      $this->__json_out($data);
      die();
    }
    ...
    $this->_setULog($dt['sess']->user->id, 13, 'pendidikan formal');
