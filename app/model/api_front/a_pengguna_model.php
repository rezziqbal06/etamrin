...
class A_Pengguna_Model extends SENE_Model
{
	...
	public function getPengguna($page = 0, $pagesize = 10, $sortCol = "id", $sortDir = "ASC", $keyword = "", $a_company_id = "", $edate = "")
	{
		...
		$this->db->select_as("$this->tbl_as.id", 'id', 0)
			...
			//  ->select_as("$this->tbl4_as.nama",'departemen',0)
			->select_as("$this->tbl_as.is_active", 'is_active', 0);
			//  ->select_as("$this->tbl_as.welcome_message",'welcome_message',0)
		;
		$this->db->from($this->tbl, $this->tbl_as);
	...
	public function getByJabatan($jabatan = "")
	{
		...
		$this->db->select_as("$this->tbl_as.id", 'id', 0)
			...
			->select_as("$this->tbl_as.email", 'email', 0)
			->select_as("$this->tbl_as.nama", 'nama', 0)
			->select_as("$this->tbl3_as.nama", 'jabatan', 0)
			->select_as("$this->tbl_as.is_active", 'is_active', 0);
			//  ->select_as("$this->tbl_as.welcome_message",'welcome_message',0)
		;
		$this->db->from($this->tbl, $this->tbl_as);
		$this->db->join($this->tbl2, $this->tbl2_as, 'a_pengguna_id', $this->tbl_as, 'id', 'left');
		$this->db->join($this->tbl3, $this->tbl3_as, 'id', $this->tbl2_as, 'a_jabatan_id', 'left');
		// $this->db->where_as("$this->tbl_as.is_karyawan", $this->db->esc(0));
		if (strlen($jabatan) > 0) {
			$this->db->where_as("$this->tbl3_as.nama", $jabatan, "OR", "%like%", 1, 1);
		}
