...
class A_Modules_Model extends SENE_Model{
	...
	public function getAll($page=0,$pagesize=10,$sortCol="identifier",$sortDir="ASC",$keyword='',$sdate='',$edate=''){
		$this->db->flushQuery();
		$this->db->select_as('identifier,name,path','path');
		$this->db->select_as('COALESCE(children_identifier,"-")','children_identifier',0);
		$this->db->select_as('level,priority,is_visible','is_visible',0);
		$this->db->from($this->tbl,$this->tbl_as);
		if(strlen($keyword)>0){
